...
apply_agent_name() {
    ...
    # azd derives its postdeploy lookup name from the service key, while the
    # platform creates the hosted agent from agent.yaml. Keep every source equal.
...
# bundled with the hosted-agent image. Unlike the Copilot agent, this agent does
# NOT read from .github/ — skills, queries, and manifests live at the agent ROOT.
# Each entry maps a repo-root-relative SOURCE to an agent-root-relative DEST.
# The repo root is the single source of truth; these copies live here only so the
# Dockerfile can COPY them.
SYNCED_ASSETS=(
...
# tree. In a bootstrapped azd wrapper the repo root is absent — the assets were
# already merged in at bootstrap time — so this becomes a no-op and the bundled
# copies stand.
sync_github_assets() {
    local root="" dir
...
sync_runtime_env() {
    # An empty Python logging level raises ValueError during container startup.
    export LOG_LEVEL="${LOG_LEVEL:-INFO}"
    azd env set LOG_LEVEL "$LOG_LEVEL"
...
set_common_env() {
    ...
    if [[ "$EXISTING_MODE" -eq 1 ]]; then
        ...
        # endpoint is derived from the project endpoint + toolbox name unless
        # explicitly supplied.
        ...
        export TOOLBOX_MCP_ENDPOINT="${TOOLBOX_MCP_ENDPOINT:-${FOUNDRY_PROJECT_ENDPOINT%/}/toolboxes/${TOOLBOX_MCP_NAME}/mcp?api-version=v1}"
...
resolve_agent_identity_context() {
    ...
    # Per-agent Entra identity display-name convention:
    #   <account>-<project>-<agent>-AgentIdentity
    AGENT_IDENTITY_DISPLAY_NAME="${account}-${project}-${agent_name}-AgentIdentity"
    log "Resolving per-agent identity '${AGENT_IDENTITY_DISPLAY_NAME}'..."

    # The identity can lag a few seconds behind deploy; retry briefly.
    AGENT_IDENTITY_OID=""
    local i
    for i in 1 2 3 4 5 6; do
        AGENT_IDENTITY_OID="$(az ad sp list --display-name "$AGENT_IDENTITY_DISPLAY_NAME" --query "[0].id" -o tsv 2>/dev/null || true)"
...
deploy() {
    ...
    if foundry_project_exists || discover_foundry_project_in_rg; then
        ...
        populate_foundry_env_from_arm || warn "Could not populate Foundry endpoint from ARM; existing-project env values may be required."
    else
        EXISTING_MODE=0
    ...
    if [[ "$need_provision" -eq 1 ]]; then
        ...
        azd provision
        # The project now exists — take its endpoint from provision outputs. This
        # OVERWRITES any value listed in .env: when provision created the account it
        # may be auto-named, so the listed endpoint is not authoritative.
        local provisioned_ep; provisioned_ep="$(azd_env_value FOUNDRY_PROJECT_ENDPOINT)"
        if [[ -n "$provisioned_ep" ]]; then
            export FOUNDRY_PROJECT_ENDPOINT="$provisioned_ep"
            log "Resolved FOUNDRY_PROJECT_ENDPOINT from provision outputs: ${FOUNDRY_PROJECT_ENDPOINT}"
        fi
    else
