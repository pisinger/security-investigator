...
| **UnifiedAgentObservability** | **Sentinel Data Lake system table** (Agent 365 / A365 Observability connector). Lake-only — NOT in Advanced Hunting or Triage MCP. Fails with `SemanticError: Failed to resolve table` under a workspace GUID; `search_tables` doesn't index it. ASIM-aligned, 69 columns covering AI agent telemetry (agent/actor identity, session/event, model + token usage, tool invocation, prompt/response forensics). Uses `TimeGenerated`. `AdditionalFields` and `EventFinishReasons` are dynamic | **Pass `workspaceId: "default"`** to `query_lake` (see Sentinel Data Lake MCP section). `parse_json()` dynamic columns before dot-access or `mv-expand` |
...
- **🔴 Lake-only / system tables — use `workspaceId: "default"`:** Tables that live in the Sentinel data lake system scope (e.g., `UnifiedAgentObservability` from the Agent 365 / A365 Observability connector) are NOT resolvable when querying against a specific workspace GUID — `query_lake` returns `SemanticError: Failed to resolve table` and `search_tables` does not index them. Pass `workspaceId: "default"` instead of the workspace GUID — this matches the portal's "Workspace Scope" → "default" dropdown option and unlocks system-tier lake tables. These tables are NOT queryable via Advanced Hunting or Triage MCP (lake-only). If a documented Sentinel data lake table fails to resolve under the workspace GUID, retry once with `workspaceId: "default"` before reporting it as missing.
